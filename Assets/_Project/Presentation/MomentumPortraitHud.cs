...
namespace IncrementalGame.Presentation
{
    public sealed partial class MomentumLabController
    {
        ...
        private void DrawLoadout()
        {
            ...
            var state=sim.Editing?"PAUSED / Rで再開":sim.Bursting?$"射出中：あと{sim.RemainingInBurst}発":sim.Balls.Count>0?$"飛行中 {sim.Balls.Count}発 / Reload {sim.ReloadRemaining:0.0}s":sim.Ready?"READY / 盤面クリックで1マガジン":sim.ChallengeState!=MomentumChallengeState.Active?"左のStageを選んで次の挑戦へ":$"Reload {sim.ReloadRemaining:0.0}s";
            ...
            Label(1146,230,410,42,p.gun==0?"太い6発 / 基礎48・初速780 / 間隔0.12秒":"小さい18発 / 基礎17・初速1050 / 間隔0.05秒",_small);
