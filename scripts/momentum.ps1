...
if ($Command -in @('build','verify')) {
    ...
    $zip = Join-Path $MomentumRun 'OneBoardMomentumLab-v0.5.4-brake-Windows-x64.zip'
    ...
    @("Version: 0.5.4-brake", "Commit: $commit", 'Unity: 6000.3.18f1', "Command: $Command", "SHA256: $hash") | Set-Content -LiteralPath (Join-Path $MomentumRun 'summary.txt') -Encoding utf8
}
