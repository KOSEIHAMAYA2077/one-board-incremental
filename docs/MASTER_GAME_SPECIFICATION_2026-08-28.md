...
- Userが「速度の減少量を増やす」と訂正・承認。Challengeの命中減速2倍／高速時間減速180/sの比較版を§33.11に採用。分裂・弾数・初速・低速域を同時に変えず，旧Buildを保持する。

### 0.8.0．2026年9月6日

- User依頼のShotgun／Sniper，B缶，マウス・キー併用操作，Help／基本Menuを§33.10へ採用。B効果は子弾へ継承しないという追加指示を優先。旧Saveを保護する新保存先へ移行。旧試作は保持。

### 0.7.5．2026年9月5日

- User承認によりRevolver0.4秒／UZI0.12秒間隔と，射出中の照準更新を§33.9へ追加。クリック一回で一マガジン，発射済み弾の非操縦，旧Build／Saveを維持。収益ギミック案はHandoffへ保存するのみ。

### 0.7.4．2026年9月5日

- User試遊の「最後に滑り続ける」指摘と調整承認により，Challengeの速度300以下を880/sで減速する止まり際Patchを§33.8へ追加。高速域120/s，停止80，再射撃300は維持。低速域の弾・尾を縮め，消滅後0.12秒の非攻撃光を表示。0.5.4-brakeとして旧Buildと仕様を保持する。
...
## 33．Challenge Arsenal（最新試作 0.5.5-steer）
...
### 33.8．低速域の急減速と収縮（0.5.4-brake）

- Userは高速域を保ち，終盤の滑走だけを短くする案を承認。Challengeでは速度300より上の時間減速120/sを保持し，300以下では880/sに切替える（INITIAL）。接触・加速がなければ300から停止閾値80まで0.25秒。旧版は約1.83秒だった。
- Tick中に300をまたぐ場合，境界まで120/s，残り時間を880/sとして速度を更新する。移動と接触検索は既存の固定Tick処理を継続。境界で速度を飛ばさず，表示だけの時間にGame Logicを置かない。
- 低速域も消滅するまでは有効な攻撃。的の抵抗，壁反射0.9，加速2倍，上限1800，分裂速度継承，10秒の親子寿命，最大弾数を維持。再加速で300を超えれば時間減速も120/sへ戻り，次の低速到達時に再び880/s。固定の消滅Timerは追加しない。
- Reload完了＋全弾300以下の再射撃条件は変えない。Gold・威力の式・Save schema・銃の初速は維持するが，低速中の移動距離と追加命中機会は減ることを意図した調整。旧ProgressなしのMomentum Labは元の120/sを保持する。
- 速度300→80で弾MeshとAuraを100→35%へ滑らかに収縮し，尾の長さ・幅・明るさも短く弱める。判定半径は縮めない。Neon／Crystal表示では消滅後，その場に最大0.12秒の収縮光だけ残す（最大64個）。非攻撃・非報酬の表示で，次射撃やClearを待たせない。Pauseでは減速・表示時間を止める。
- 旧0.5.3 Build，Crystal Kit V1，Gallery，記録を保存。比較・調整の対象は止まり際であり，新効果・成長曲線・音の変更を混ぜない。

### 33.9．射出中の照準と遅い発射レート（0.5.5-steer）

- User承認により，ChallengeのRevolverを0.4秒間隔・6発（最初から最後2.0秒），UZIを0.12秒間隔・18発（2.04秒）に更新。いずれもINITIAL。固定60tick/sのため0.12秒間隔は直後のTickへ量子化するが，次射出予定時刻への加算で累積ずれを防ぐ。
- 一クリックでマガジンを開始し，長押しを要求せず最後まで射出。射出中のマウス照準を次の弾の方向へ反映する。発射済みの弾・分裂弾を操縦しない。Seed付き±2度Spreadは各射出時に現在方向へ適用。同じSeedと同じ時刻の照準入力列なら結果を再現できる。
- 照準更新は盤面内・Gunから30以上離れた有限座標のみ。カーソルが左右UI／盤面外へ出る，Gunに近すぎるなどの無効入力では最後の有効方向を保持する。Pause中・非Focus・射出終了後・Pauseを閉じた直後の入力防止Frameでは射出方向を更新しない。銃身の表示と次弾の発射方向を一致させる。
- 射出中の追加クリックを次マガジンとして予約しない。弾数，初速，威力，低速急減速，再射撃300，親子10秒寿命を保持。最後の射出から0.8秒Reload。親子寿命は最初の射出から10秒のままで，後の弾の可動時間を延長しない。
- 旧ProgressなしのMomentum Labは0.12秒の三発・開始時照準固定を保持。Save schemaと恒久進行，旧Build・Gallery・Kitは削除しない。
- ワープ穴・蓄積穴・変換穴・条件付きバンパーGold・スキルツリーは未実装候補。Handoffの例示報酬+2kを実装値にしない。本Patchは射撃操作の変更に限定する。

### 33.10．Arsenal・条件付きB缶・基本操作（0.6.0-arsenal）

2026年9月6日のUser依頼による追加。最新試作ではこの節が旧操作・2銃制限・保存先に優先する。以下の新しい数値はINITIAL。

- Shotgun：1マガジンで8粒を同時射出。照準を中心に−24〜+24度へ等間隔，各粒にSeed付き±2度。半径6，初速760，基礎威力18。Sniper：1マガジン1発，半径14，初速1650，基礎威力90，散布±0.4度。両方を最初から選択可能にして比較する。Revolver6発／0.4秒，UZI18発／0.12秒と40G開放は保持。銃の外観変更は今回は行わない。
- 全銃で現在の装備効果，威力×速度比，速度上限1800，再射撃300，0.8秒Reload，10秒絶対寿命，256同時／1024生成を共用。初速強化適用直後も1800へClamp。Shotgunの8粒で携行1マガジンを消費し，最終射出と同時にReload開始。既存の低速減速880/sは今回変更しない。
- R／ホイールで所有銃を循環（未開放UZIは飛ばす）。右の銃Buttonでも選択可。飛行中・射出中の切替は次のマガジン用選択だけを変更し，実行中の銃・弾数・間隔・装備Snapshotを変えない。装備効果・強化の付け替えは従来どおり攻撃終了後。
- マウス移動で照準，左クリックで1マガジン。Q/E/Spaceを押すとKeyboard操作へ切替え，Q/Eは左右120度/s，Spaceの押下1回で発射。盤面内のマウス移動またはクリックでMouse操作に戻る。静止カーソルはKeyboard操作を奪わない。Keyboardでも射出中の次弾へ照準反映。無効Aim・UI・非Focus・Pause・閉じた直後の誤射防止を維持。長押し予約なし。Spaceの旧回収操作は右Button／Menuへ移す。
- B缶：各Challenge開始時に(660,620)／(940,620)へ2個，半径18。黄緑色の缶＋Bロゴ。Swept Circle接触で消費し，その弾だけをB状態にする。取得だけではGoldなし。B状態の弾が紫バンパーに当たると+12Gを即時獲得し，Bを解除。壁／的では換金せず，消滅・回収でも報酬なし。所持中は別の缶を消費せず，換金後は再取得可。缶は次Challengeまで復活しない。接触同時刻の優先順は的→バンパー→壁→加速ゾーン→缶→同種ID順。
- User追加指示によりB状態は分裂した子弾へ継承しない。分裂で親が消える場合は未換金Bも失われる。ただし子自身が缶を取れば通常どおりB状態になる。Goldenや既存装備効果の継承は従来どおり。B状態は黄緑色とB文字で表示。缶を破壊して一時ゾーンを作る代案はDEFERRED。
- 左上?／F1はHelp，≡は設定Menu。Help／Menu／終了確認中はSimulation停止，背景UIと攻撃入力を遮断。Escで終了確認，Enter／終了Buttonで終了，キャンセルで復帰。終了時に既存の永続進行を保存し，飛行中状態は保存しない。SE音量0〜100%，表示ThemeはCrystal／Neon／Diagram。BGMは未実装と明記。SEとThemeのみ端末PlayerPrefsの専用Keyへ保存する。
- 保存先は新規challenge-arsenal-v1.jsonとchallenge-arsenal-history。新Fileが存在しない初回のみ旧challenge-v1.jsonの妥当な値を読み込む。旧Save／履歴は書き換えない。銃ID0〜3とPresetを検証し，破損／未知Schema時は元File保護と未保存警告を維持。旧Build／Gallery／Assetは削除しない。

### 33.11．狙う意味を比較する強い減速（0.6.1-drag）

- User承認の意図は「速度の減少量を増やす／減速を強める」。ChallengeでTargetに当たった時の速度損失を従来の2倍にする（INITIAL）。通常Targetは150→300，装甲Targetは420→840。抵抗軽減装備1/4は維持し，装備時の損失は75／210。HP・撃破報酬・当たり判定は変更しない。
- Challengeの速度300より上の時間減速を120/s→180/sにする（INITIAL）。300以下は従来どおり880/s，停止閾値80で，接触がなければ300→80は約0.25秒。Tick途中で300へ到達した場合は境界前後で時間を分けて適用する。
- 命中威力は速度損失の適用前の速度から計算する。初速・弾数・発射間隔・壁とバンパーの速度×0.9・加速ゾーン×2と上限1800・分裂継承／回数・B缶・再射撃条件・Gold経済・Save形式を維持する。経路後半の速度低下で威力や再分裂の機会が減るのはこの比較Patchの意図。
- Progressなしの旧Momentum Labは時間減速120/s・命中損失150／420を維持。旧Buildと記録を削除せず，直前0.6.0との比較に使う。今回の版も同じArsenal Save形式で，新しい恒久値は保存しない。
- 数値だけで「狙いが面白くなった」と判定しない。同Seed・同構成・同射線の1マガジン計測で，平均撃破数・生存弾Peak・攻撃解決時間・射線による撃破数差を旧版と比較する。手動照準の楽しさはUser試遊で判断する。360度採掘・回収・帰還・拠点生産・荷物整理は別構想の未採用メモであり，今回実装しない。
